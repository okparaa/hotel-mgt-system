...
export const BookingCheckout = ({}: BookingCheckoutProps) => {
  ...
  const [bookRes, bookerMutate] = useBookerMutation();
  ...
  useEffect(() => {
    ...
    bookRes.error && setErrorMsg(() => errorMsgHandler(bookRes.error)?.message);
  }, [bookRes.data, bookRes.error]);
    ...
    <div>
      ...
      <div className="flex border-2 border-slate-600 py-2 mt-2 rounded-md">
        <div className="flex-1 text-center checkout">
          <span>Cash</span>
          <span
            className="border-y cursor-text border-black flex w-10/12 m-auto justify-center items-center rounded-md h-10"
            onClick={(e) => {
              addInput(e, (value) => {
                updateChest({
                  type: "booker",
        ...
        <div className="flex-1 text-center">
          <span>POS</span>
          <span
            className="border-y cursor-text border-black flex w-10/12 h-10 m-auto justify-center items-center rounded-md"
            onClick={(e) => {
              addInput(e, (value) => {
                updateChest({
                  type: "booker",
        ...
        <div className="flex-1 text-center">
          <span>Total</span>
          <span className="bwks bg-slate-600 text-white relative font-semibold border border-black flex w-20 h-10 m-auto justify-center items-center rounded-md">
            {toCommas(total)}
            {total !== booker.total ? (
